...
public class BunkerCreator : MonoBehaviour {
	...
	void createBunkerLine(Texture bunkerTexture, float y) {
		...
		for (int i = 0; i < 4; i++) {	
			GameObject bunker = (GameObject) GameObject.Instantiate(bunkerPrefab, new Vector3(distanceBetweenBunkers * (i - 2) + distanceBetweenBunkers/2, y, 400), Quaternion.identity);
